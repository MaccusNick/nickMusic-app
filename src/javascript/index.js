...
class Player {
  ...
  start() {
    fetch("https://maccusnick.github.io/data-mock/nick-music/music-list.json")
      .then((res) => res.json())
      .then((data) => {
        this.songList = data;
        this.loadSong();
  ...
  setLyricToCenter(node) {
    let translateY = node.offsetTop - this.$(".panel-lyric").offsetHeight / 2;
    translateY = translateY > 0 ? translateY : 0;
    this.$(
      ".panel-lyric .container"
    ).style.transform = `translateY(-${translateY}px)`;
    this.$$(".panel-lyric p").forEach((node) =>
